...
class DeitFineTuneTiny(nn.Module):
    def __init__(self, num_labels, from_saved_weights=None, num_tasks=1, image_size=224, return_features=False, **kwargs):
        super(DeitFineTuneTiny, self).__init__()
        self.beit = ViTModel.from_pretrained('facebook/deit-tiny-patch16-224')
        self.return_features = return_features  # Control flag for returning features before classification
        ...
        if not self.return_features:
            if num_tasks > 1:
                self.classifiers = nn.ModuleList([nn.Linear(192, n_labels) for n_labels in num_labels])
            else:
                self.classifier = nn.Linear(192, num_labels[0])
    
    def forward(self, pixel_values):
        # Process the entire batch through BEiT
        outputs = self.beit(pixel_values=pixel_values)
        pooled_output = outputs.pooler_output  # Batched pooled output

        if self.return_features:
            # If returning features, skip classification
